...
class IACEventClassifier(BaseEventClassifier):
    ...
    def _initialize_rules(self):
        ...
        self.safe_read_only.update({
            ...
            ("cloudformation.amazonaws.com", "ListStacks"),                 # ✅ Stack inventory - infrastructure overview (administrative inventory, not exploitable - should be safe read)

            # CodeBuild - Build information
            ("codebuild.amazonaws.com", "ListProjects"),                    # ✅ Project inventory - available build projects (administrative inventory, not exploitable - should be safe read)
            ("codebuild.amazonaws.com", "ListSandboxesForProject"),         # ✅ Project sandboxes - isolated build environments (administrative inventory, not exploitable - should be safe read)
            ("codepipeline.amazonaws.com", "ListPipelines"),                 # ✅ Pipeline inventory - available workflows (administrative inventory, not exploitable - should be safe read)

            # Tagging - Tag information
            ("tagging.amazonaws.com", "GetTagKeys"),                        # ✅ Available tag keys - tagging schema (administrative schema info, not exploitable - should be safe read)
            ("tagging.amazonaws.com", "GetTagValues"),                      # ✅ Tag values for specific keys - tagging data (administrative metadata, not exploitable - should be safe read)
            
            # Resource Groups - Resource organization
            ("resource-groups.amazonaws.com", "ListGroups"),                # ✅ Resource group inventory - organizational structure (administrative inventory, not exploitable - should be safe read)

            # Support - Support information
            ("support.amazonaws.com", "DescribeServices"),                 # ✅ Support services - available support categories (administrative service list, not exploitable - should be safe read)
            ("support.amazonaws.com", "DescribeSeverityLevels"),           # ✅ Support severity levels - case priority options (administrative metadata, not exploitable - should be safe read)
            ("support.amazonaws.com", "DescribeCreateCaseOptions"),        # ✅ Case creation options - support request setup (administrative metadata, not exploitable - should be safe read)
        })
